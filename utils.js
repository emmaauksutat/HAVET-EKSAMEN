...
//nbcols = number af colliner?? nbrows = rækker
export function getFramesPos(nbCols, nbRows, tileWidth, tileHeight) {
  const framesPos = []; //et array med objekter, der alle indeholder y og x positioner
  let currentTileX = 0;
  let currentTileY = 0;
...
export function drawTile(
  p, //p5 
  src, //referencen
  destinationX, //hvor skal spriten være på vores canvas
  destinationY,
  srcX, //koordinaterne i billedet
  srcY,
  tileWidth, //så p5 ved hvor den skal cutte vores billede ud
  tileHeight
) {
  p.image(
    src,
